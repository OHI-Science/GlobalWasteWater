...
if __name__ == '__main__':
    try:
        filename = sys.argv[1] # CPT - file name of pourpoint shape file to split up
        split_num = int(sys.argv[2]) # CPT - # of rows of split files (with 1000 pp, 500 will make two new files w rows)
    except:
    ...
    for i, fn in enumerate(fn_list):
        cmd = "v.in.ogr -o input={} output=pourpnts{}".format(fn, i+1) #added flag to overwrite projection
        print(cmd)
        os.popen(cmd)
