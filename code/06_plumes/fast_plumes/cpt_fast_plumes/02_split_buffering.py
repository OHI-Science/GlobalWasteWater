...
Updated by Cascade Tuholske Feb 2021
"""
...
# ocean_mask_fn = os.path.join(ww_dir, "intermediate_files", "ocean_masks/", "ocean_mask_landnull.tif") # CPT updated
...
def setup_grass(pnt_file):
  ...
  cmd = "v.in.ogr -o input={} output={}".format(pnt_file, shortname) #ignore projection
  call(cmd, shell=True)
...
def run_buffering(shortname):
  ...
  """
  cmd = "sh ./run_subset_plumes.sh {} > split_run_{}.log &".format(shortname, shortname) #CPT use screen, not nohups
...
if __name__ == '__main__':
  try:
    ...
    if len(sys.argv) == 3:
      ...
      file_pattern = "effluent_N_pourpoints_plumes_test1000_*.shp" #"pour_points_*.shp" CPT
  except:
